...
void find_line() {
  ...
  if (!line_y_found) {
    ...
    line_y_found = true;
    stop_motor();
    move(0, HANDLE_DISPLACEMENT_Y + handle_displacement);
    up_handle();
...
void draw_dS_line(double start_dS_x, double start_dS_y, double dS_x, double dS_y) {
  up_handle();
  move(start_dS_x + sign(dS_x) * LINE_HEIGHT, start_dS_y + sign(dS_y) * LINE_HEIGHT);
  down_handle();
  move(dS_x - sign(dS_x) * LINE_HEIGHT, dS_y - sign(dS_y) * LINE_HEIGHT);
  up_handle();
}
void draw_length_line(double start_length, double start_angle, double length, double angle) {
  angle = angle / 180 * PI;
  start_angle = start_angle / 180 * PI;
  up_handle();
  move(start_length * cos(angle), start_length * sin(angle));
  down_handle();
  move(length * cos(angle), length * sin(angle));
  up_handle();
}
...
void draw_function(double center_x, double center_y, double scale = 1) {
  ...
  for (double x = -1; x < 1; x += 0.025) {
    const_part = 0;
    changing_part = sqrt((-2 * x * x - 1 + sqrt(8 * x * x + 1)) / 2);
    if (!start) {
      set_pos(center_x + x * scale, center_y - (const_part + changing_part) * scale);
      down_handle();
      start = true;
...
void find_point(int dir, double &center_x, double &center_y, double k = 0.3) {
  static double top, bottom, left, right;
  //нахождение середины по х
  //set_pos(x_curr, (bottom + top) / 2);
  move(3 * dir, 0);
  while (analogRead(LINE_SENSOR) > line_value * k) {
    Vx = 10 * dir;
    wait();
  }
  left = x_curr;
  ...
  stop_motor();
  right = x_curr;
  center_x = (left + right) / 2 + HANDLE_DISPLACEMENT_X /* * (dir == 1)*/;
  ...
  bottom = y_curr;
  center_y = (bottom + top) / 2 + HANDLE_DISPLACEMENT_Y + handle_displacement + 3.5;
