...
/*double get_angle(double size_x, double size_y){
  return atan(size_y / size_x) / PI * 180;
}*/
double get_line_sens_pos_x(bool move_left=false) {
  Serial.println(x_curr - HANDLE_DISPLACEMENT_X - 4 * move_left);
  return x_curr - HANDLE_DISPLACEMENT_X - 4 * move_left;
}
double get_line_sens_pos_y(bool move_up=false) {
  Serial.println(y_curr - handle_displacement - 11.5 - 10 * move_up);
  return y_curr - handle_displacement - 11.5 - 10 * move_up;
}
void set_pos(double x, double y) {
  ...
  while (((x - x_curr) * sign(dS_x) > 0.25 || (y - y_curr) * sign(dS_y) > 0.25)) {
...
void set_pos_line_sens(double x, double y) {
  set_pos(x + HANDLE_DISPLACEMENT_X, y + handle_displacement + 9.5);
}
